...
#  ---------------------------------- METODO DE NEWTON ---------------------------------- 
def newton(funcion_objetivo, x, epsilon1, epsilon2, max_iterations):
    terminar = False
    ...
    while not terminar:
        ...
        if norm(gradienteX) < epsilon1 or k >= max_iterations:
            terminar = True
        else:
            ...
            if det(hessian) == 0:
                print("La Hessiana no es invertible, reiniciando con un nuevo punto.")
                xk = np.random.rand(len(x))
                k += 1
                continue
            ...
            if np.dot(gradienteX, punto) >= 0:
                print("No es una dirección de descenso, reiniciando con un nuevo punto.")
                xk = np.random.rand(len(x))
                k += 1
                continue
            ...
            if (norm(x_k1 - xk) / (norm(xk) + 0.00001)) <= epsilon2:
                terminar = True
            else:
                k += 1
                xk = x_k1
...
punto_final = newton(funcion_objetivo, prueba, epsilon1, epsilon2, max_iterations)
print("Punto final:", punto_final)
print("Valor de la función objetivo en el punto final:", funcion_objetivo(punto_final))
